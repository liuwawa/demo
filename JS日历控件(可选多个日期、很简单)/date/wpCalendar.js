...
Ver 2.60
修改日期：2015-11-30
1.完善多日期选择操作时的样式添加以及input框的数据优化等。
2.存在问题：样式在两个浏览器还不一致，以及html页面添加相应的<!DOCTYPE> 声明后在非IE下日历样式不给力。
注：（由于各种插件在本人现做项目中都不兼容ie，因是新手，故虽有不足，也只能勉强使用此插件，其他不足等待后期完善，如你完善的更好，望请指点。qq：1789861010  或传至同名qq邮箱,提前谢过！）
...
function wpCalendar(){
	//==================================================== 参数设定部分 =======================================================
	this.bmoveable	= false;			//设置日历是否可以拖动
	this.datestyle	= "yyyy-MM-dd";		//added in Ver 2.1
	this.cellwidth	= 25;			//设置单个单元格的宽度
	...
	this.redraw		= function (){				//定义redraw方法，用于重绘整个日历
		//==================================================== WEB 页面显示部分 =====================================================
		if(this.cellwidth<25)this.cellwidth=25;
	...
	this.writeCalendar=function (){	//主要的写程序**********
		...
		for (i=0; i<39; i++)
		{
			...
			//初始化给选择的日期提供样式
			var dates = this.tgtObject.value.split(",");
			if(dates.join("").length==0||dates[dates.length-1]==""){
				dates.pop();
			}
			for(j in dates){
				if(dates[j] == this.Days[i].date){
					var da = this.DateCell[i];
...
//对日期的处理就是在这里完成的
	...
	this.dateCellClick=function (n){  //点击显示框选取日期，主输入函数*************
		var judge = -1;//判断有没有重复选取项的标志（-1：没有 、其他：有）
		var dates = this.tgtObject.value.split(",");
		if(dates.join("").length==0||dates[dates.length-1]==""){
			dates.pop();
		}
		...
		for(i in dates){
			if(dates[i] == this.Days[n].date){
				judge = i;  //判断有没有重复选择日期
				var da = this.DateCell[n];
				if(judge!=-1){
					this.DateCell[n].className="normalday";
				}else{
					this.DateCell[n].className="selectedday";
				}
			}
		}
		if(judge!=-1){//把重复选择的那一项删掉
			tt = dates[judge];
			dates[judge]=dates[dates.length-1];
			dates[dates.length-1] = tt;
			dates.pop();
			this.tgtObject.value = dates.join(",");
			var da = this.DateCell[n];
			this.DateCell[n].className="normalday";
			this.tgtObject.blur();
		}
		...
		if (this.tgtObject){
			if(judge == -1){ //如果没有重复添加日期	
				if(dates.length==0){
					this.tgtObject.value+= this.Days[n].date;	//modified in Ver 2.51 设置input框中显示的日期拼接格式
				}else{
					this.tgtObject.value+=","+ this.Days[n].date;	//modified in Ver 2.51 设置input框中显示的日期拼接格式
				}
				var da = this.DateCell[n];
...
var theme=new Object();
	...
	theme["Classic"].borderColor		= "#D6D6D6";	//边框颜色
	theme["Classic"].borderColorDark	= "#D6D6D6";	//边框暗部颜色
	theme["Classic"].bgColorMain		= "#BDEBEE";	//日历主背景色
	theme["Classic"].bgColorLight		= "#FFF8EC";	//日历的浅背景色
	theme["Classic"].headBgColor		= "#BDEBEE";	//日历头部背景色
	...
	theme["Classic"].buttonBorderColor	= "#BDEBEE";	//按钮边框颜色
	theme["Classic"].buttonColor		= "#BDEBEE";	//按钮的主色调
	theme["Classic"].buttonFontColor	= "#000000";	//按钮文字颜色
	theme["Classic"].dragBarFontColor	= "#000000";	//拖动条的文字颜色//星期的文字颜色
